...
class Relatorios extends MY_Controller
{
        ...
        public function comissaoOs()
    {
        ...
        if ($this->input->post()) {
            ...
            $de = $this->input->post('finalizado_de') ?: date('Y-m-d\TH:i', strtotime('-1 month'));
            $ate = $this->input->post('finalizado_ate') ?: date('Y-m-d\TH:i');
            $manPrevnt = $this->input->post('manPrevnt');
            ...
            if (!empty($de)) {
                $de = explode('-', $de);
                $de2 = explode('T', $de[2]);
                $de = $de[0] . '-' . $de[1] . '-' . $de2[0] . ' ' . $de2[1];
                $where_array['de'] = $de;
            }

            if (!empty($ate)) {
                $ate = explode('-', $ate);
                $ate2 = explode('T', $ate[2]);
                $ate = $ate[0] . '-' . $ate[1] . '-' . $ate2[0] . ' ' . $ate2[1];
                $where_array['ate'] = $ate;
            }


            $data = $this->data['results'] = $this->os_model->getOsComissao(
                'os',
                'os.*,                   
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $dt) {
                    ...
                    if (!empty($this->input->post('tecnico'))) {
                        $totaltecnico += (float)$dt->totalServicos;
                    }
                }
                ...
                foreach ($data as $d) {
                    ...
                    $data3[$c]['vendedor'] = !empty($d->vendedor) ? $d->vendedor : "";
                    ...
                    $data3[$c]['tecnicoName'] = !empty($d->tecnicoName) ? $d->tecnicoName : "";
                }
            ...
            $this->data['inputGet'] = $this->input->post();
            $this->data['totaltecnico'] = $totaltecnico;
            $this->data['totalLancamentos'] = $totalLancamentos;
            $this->data['totalProdutos'] = $totalProdutos;

            $this->data['view'] = 'relatorios/rel_comissao_servicos';
            return $this->layout();
        }
    ...
    public function comissaoServicosImprimir()
    {
        /*  if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rServico')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de serviços.');
            redirect(base_url());
        ...
        $data['usuario'] = $this->mapos_model->getById($this->session->userdata('id_admin'));
        $data['usuarios'] = $this->contas_model->getAll('usuarios');
        $data['inputGet'] = $this->input->post();
        $data['totaltecnico'] = $totaltecnico;
        $data['totalLancamentos'] = $totalLancamentos;
        $data['totalProdutos'] = $totalProdutos;
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Comisão de serviços';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirComissaoServicos', $data, true);
        pdf_create($html, 'relatorio_servicos' . date('d/m/y'), true);
    }
    public function comissaoVendas()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->data['view'] = 'relatorios/rel_comissao_vendas';
            return $this->layout();
        }
    ...
    public function comissaoVendasImprimir()
    {
        /*  if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rServico')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de serviços.');
            redirect(base_url());
        ...
        /* echo "<pre> comissaoServicosImprimir :<br>";
        print_r($this->input->post());
        echo "</pre>";
        exit; */
        ...
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Comissão de Vendas';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirComissaoVendas', $data, true);
        pdf_create($html, 'relatorio_servicos' . date('d/m/y'), true);
    }
