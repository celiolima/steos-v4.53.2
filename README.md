...
Bem-vindo ao **SteosMapos**, uma solução de Sistema de Ordem de Serviço (Mapos) atualizada e superpoderosa. Este projeto combina a base oficial e mais recente do Mapos v4.53.2 com os módulos exclusivos criados sob medida para o "Steos".

## Visão Geral do Sistema
O **SteosMapos** foi desenhado para oferecer a estabilidade da versão mais moderna do framework CodeIgniter e Bootstrap (trazidos na v4.53.2 oficial), enquanto mantém a inteligência de negócios customizada:
...
1. **Baixe o Projeto:** Extraia a pasta `steosMapos v4.53.2` no diretório raiz do seu servidor web (Ex: `htdocs` no XAMPP ou `www` no WAMP).
2. **Configuração do Banco de Dados:**
   - Crie um banco de dados no MySQL (ex: `mapos`).
...
4. **Base URL:**
   - Se estiver usando em localhost, garanta que a URL base no arquivo `application/config/config.php` esteja apontando corretamente (ex: `http://localhost/steosMapos/`).
...
As customizações exclusivas deste projeto em relação ao Mapos oficial encontram-se principalmente nos arquivos:
